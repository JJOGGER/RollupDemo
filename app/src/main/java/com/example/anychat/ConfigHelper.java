...
public class ConfigHelper {
	...
	public ConfigEntity LoadConfig(Context context) {
    	...
    	SharedPreferences  share = context.getSharedPreferences("perference", 2);  
        ...
        configEntity.ip = Utils.ANYCHAT_DOMAIN;//"demo.anychat.cn"
        configEntity.port = Utils.ANYCHAT_PORT;//8906--anychat测试服务器端口
        
        configEntity.configMode = share.getInt("configMode", ConfigEntity.VIDEO_MODE_CUSTOMCONFIG);
        configEntity.resolution_width = 352;
        configEntity.resolution_height = 288;
	...
	{
		ConfigEntity configEntity = ConfigHelper.getConfigHelper().LoadConfig(context); //读取配置对象
